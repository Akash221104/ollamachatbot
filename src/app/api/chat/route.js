...
async function getAvailableModelNames() {
  try {
    const response = await fetch(`${OLLAMA_BASE_URL}/api/tags`, {
      method: 'GET',
      next: { revalidate: 10 } // Cache model list briefly
    });
...
export async function POST(request) {
  try {
    ...
    const systemPrompt = `You are a helpful and intelligent assistant for C-DAC Revival DR products.

--- DOCUMENTATION ---
${context}
--- END DOCUMENTATION ---

Rely ONLY on the provided documentation to answer questions. Do not assume or extrapolate anything not explicitly mentioned.

To ensure fast responses:
1. Keep your answers extremely concise, direct, and to the point (maximum 3 sentences or a short bulleted list).
2. Avoid any introductory or concluding conversational filler (do not repeat the question).

Scope Guidelines:
- If the user greets you or engages in simple conversation, respond naturally but briefly.
- If the question is NOT related to C-DAC Revival Disaster Recovery products or the facts in the documentation, you MUST output EXACTLY this sentence and nothing else:
...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ status: 'offline', error: 'Ollama returned error' });
    }
